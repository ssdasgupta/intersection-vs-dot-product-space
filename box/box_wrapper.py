...
class BoxTensor(object):
    ...
    def intersection(self: TBoxTensor,
                     other: TBoxTensor,
                     intersection_temp: float = 1.0,
                     bayesian: bool = False) -> TBoxTensor:
    ...
    def _log_soft_volume_adjusted(
       ...
       #  eps = torch.finfo(z.dtype).tiny  # type: ignore
        ...
        return (
            torch.sum(
                torch.log(
                    F.softplus(Z - z - 2 * euler_gamma * intersection_temp, beta=volume_temp) + 1e-23
                ),
                dim=-1,
            )
        )
    ...
    def gumbel_intersection_log_volume(
        ...
        z, Z = self._intersection(other,
                                  intersection_temp=intersection_temp,
                                  bayesian=True)
        vol = self._log_soft_volume_adjusted(z,
                                             Z,
    ...
    def log_gumbel_membership(
        ...
        log_membership = -torch.exp((v - self.Z)/membership_temp).sum(axis=-1) - torch.exp(-(v - self.z)/membership_temp).sum(axis=-1)
