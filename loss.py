...
def nll(pos, neg, **kwagrs):
    ...
    return -(pos.sum() + log1mexp(neg).sum())
...
def nce(pos, neg, **kwagrs):
	...
	"""
    return -(F.logsigmoid(pos) + torch.sum(F.logsigmoid(-neg), dim=1)).sum()
...
def bpr(pos, neg, **kwagrs):
	"""
	This is Bayesian Personalised Ranking loss.
	Here, the input scores can be un-normalised. The object here
	is to make increse the pos similarity score more than the negative
	scores. If that is satisfied then the loss is zero.

	Args:
	    pos: Unnormalised similarity score for positives.
	    neg: Unnormalised similarity score for negatives.
	Output:
	    loss =  softplus(mean(neg) - pos)
	"""
	return torch.sum(torch.nn.functional.softplus(torch.mean(neg) - pos))
